...
public class OnlineAccounts.AccountView : Gtk.Grid {
    ...
    public AccountView (OnlineAccounts.Account plugin) {
        ...
        var account = plugin.account_service.account;
        var auth_data = plugin.account_service.get_auth_data ();
        ...
        var provider = account.manager.get_provider (account.get_provider_name ());
        ...
        var user_label = new Gtk.Label (Markup.escape_text (account.get_display_name () ?? _("New Account")));
        ...
        account.list_services ().foreach ((service) => {
            if (account.manager.list_applications_by_service (service).length () == 0)
                return;
            ...
            service_switch.active = account.get_enabled ();

            var acl_list = new ACListBox (account, service, identity);
        ...
        if (i == 1) {
            var provider_name = account.manager.get_provider (account.get_provider_name ()).get_display_name ();
        ...
        account.display_name_changed.connect (() => {
            user_label.label = Markup.escape_text (account.get_display_name () ?? _("New Account"));
        });
    }
