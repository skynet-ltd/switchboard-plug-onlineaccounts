...
public class OnlineAccounts.Plugins.MailAccount : OnlineAccounts.Account {
    ...
    public MailAccount (Ag.Account account, bool is_new = false) {
    ...
    public async void authenticate (Signon.Identity identity, uint32 id) {
        ...
        try {
            var session = identity.create_session ("mail");
            session_result = yield session.process_async (session_data, "mail", null);
            var access_token = session_result.lookup_value ("ImapPassword", null).dup_string ();
            info.set_secret (access_token, true);
            var username = session_result.lookup_value ("ImapUser", null).dup_string ();
